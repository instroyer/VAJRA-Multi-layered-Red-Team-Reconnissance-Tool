...
def process_targets(targets, module_choices, report_enabled, is_file_input=False, file_name=None):
    ...
    for target in targets:
        ...
        # Call report generation if enabled
        if report_enabled:
    ...
    try:
        ...
        while True:
            ...
            while True:
                ...
                # Get report preference, skipping for certain modules
                choices_set = set(module_choices.split())
                ...
                if module_choices == '6': # Eyewitness only - skip report prompt
                    ...
                    report_enabled = False
                elif choices_set and choices_set.issubset({'2', '3'}):
                    # If only Subfinder and/or Amass are selected, no report is generated
                    info("Report generation is not applicable for Subfinder/Amass only scans.")
                    report_enabled = False
                else:
                    try:
                        generate_report_choice = get_input("\nDo you want to generate a report for this? (y/n)> ").strip().lower()
                        report_enabled = (generate_report_choice == 'y')
                    except KeyboardInterrupt:
                        info("\nOperation cancelled by user.")
